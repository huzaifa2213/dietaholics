            ...
            if ($output->results[0] && $output->results[0]->geometry && $output->results[0]->geometry->location && $output->results[0]->geometry->location->lat) {
                ...
                if (isset($_FILES['image']) && $_FILES['image'] != '') {
                    ...
                    for ($pi = 0; $pi < $fileCount; $pi++) {
                        ...
                        $img = $this->request->getFileMultiple('image')[$pi];
                        if ($_FILES["image"]['error'][$pi] <= 0) {
                ...
                if (isset($imgGrr)) {
                    $data_array['profile_image'] = implode(',', $imgGrr);
                }
    ...
    public function edit($id)
    {
        ...
        if ($_POST && !empty($_POST)) {
            ...
            if ($output->results[0] && $output->results[0]->geometry && $output->results[0]->geometry->location && $output->results[0]->geometry->location->lat) {
                ...
                $data_array['latitude'] = $latitude;
                $data_array['longitude'] = $longitude;
                $imgGrr = isset($_POST['happening_gallery_image']) && $_POST['happening_gallery_image'] != '' ? $_POST['happening_gallery_image'] : array();
                if (isset($_FILES['image']) && $_FILES['image'] != '') {
                    $fileCount = count($_FILES["image"]['name']);
